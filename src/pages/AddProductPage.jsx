  ...
  const addProduct = async (productObj) => {
    try {
      ...
      for (let i = 0; i < productObj.images.length; i++) {
        formData.append(`images`, productObj.images[i]);
      }

      // Append thumbnail file to the FormData object
      formData.append("images", productObj.thumbnail);
      // default rating to 1
      ...
      const { data } = await axios.post(
        process.env.REACT_APP_SERVER_BASE_URL + `/v1/products/add`,
        formData,
        {
          headers: {
          ...
          <form
            ...
            onSubmit={handleSubmit((data) => {
              if (thumbnail === "") {
                ...
                addProduct({ ...data, thumbnail, images });
              }
            })}
                        ...
                        onChange={(e) => {
                          ...
                          if (files.length > 5) {
                            ...
                            if (images.length >= 5) {
                              ...
                              Toasts(
                                "error",
                                "Cannot select more than 1 image"
                              );
                            } else if (images.length >= 3 && files.length > 2) {
                              Toasts(
                                "error",
                                "Cannot select more than 2 image"
                              );
                            } else if (images.length >= 2 && files.length > 3) {
                              Toasts(
                                "error",
                                "Cannot select more than 3 image"
                              );
                            } else if (images.length >= 1 && files.length > 4) {
                              Toasts(
                                "error",
                                "Cannot select more than 4 image"
                              );
                            } else {
                    ...
                    <div className="flex flex-col gap-2 w-full">
                      ...
                      <input
                        ...
                        {...register("discountPercentage", {
                          required:
                            "DiscountPercentage must be between 0 to 90",
                        })}
                      ></input>
                    </div>
                    <div className="flex flex-col gap-2 w-full">
